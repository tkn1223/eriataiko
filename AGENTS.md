...
**次に何をやるかは `docs/roadmap.md` から選ぶ。** 残っていること・おすすめの順番・
誰がやるかが 1 枚にまとまっている。新しい作業に入る前に必ず読む。終わったら更新する。

---

## 役割分担は無い

**実装する人が最後まで通す。** 画面・入口（API）・データの読み書きを、別々の人に分けない。

「画面は作った。入口につなぐのは別の人」という切り方をすると、
**どちらの担当でもない部分が残る。**

実際に起きた: 「得点を入れる画面」と「得点を保存する入口」が両方できているのに、
**その 2 つをつなぐ人がいないまま**、押した点が保存されない状態が続いた。

古い仕様書に「フロントは別の人が担当する」「◯◯側の担当」と書いてある箇所がある。
**あれはもう無効。** 該当ファイルの先頭に打ち消しを書いてある。
