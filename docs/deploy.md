...
```bash
npm run release:status      # いま何が出ていないか見る
```

1. **`develop` → `main` の PR を作る**
2. テストが緑になったら **`Create a merge commit`** でマージ
   （`Rebase and merge` は使わない。理由は `AGENTS.md`）
3. `main` のテストが緑になるのを待つ（数分）
4. **Actions →「公開する」を `main` から押す**
5. `npm run release:status` をもう一度打って、何も出ないことを確かめる

### 「ずれていない」を何で見るか

**ファイルの中身で見ます。コミットの並びは見ません。**

`develop` → `main` はマージコミットで入れるので、`main` にリリースのたびに 1 つ
コミットが増えます。**この 2 本のコミットの並びは永久に一致しません。** それは害が無いので
気にしないでください。

大事なのは 2 つだけで、`npm run release:status` がその両方を見ます。

| 見るもの | あるべき状態 |
| --- | --- |
| `main` に出していない変更 | リリース後は「なし」 |
| **`main` にだけある変更** | **常に「なし」**（あったら `main` に直接コミットしている） |

### マージしただけではサイトは変わりません

`main` は「本番に出してよいコードの置き場」であって、サイトそのものではありません。

```
develop  ──マージ──►  main  ──公開ボタン──►  https://eriataiko.vercel.app
         (GitHub の中)        (ここで初めて世に出る)
```

| 変えたもの | マージ | 公開 |
| --- | --- | --- |
| アプリのコード（`src/`） | 要る | **要る** |
| ワークフロー（`.github/`） | 要る | 要らない（GitHub 上で動くので） |
| ドキュメント（`docs/`） | 要る | 要らない |
...
`main` には PR とテストを通ったものしか入らないので、`main` から出すものは
そのまま本番に出してよいものです。だから選ばせる必要がありません。
...
**この確認を飛ばす手段はありません。** 逃げ道を作ると、いつか「とりあえず飛ばす」が
習慣になって、赤いまま本番に出ます。テストが緑になるのを待ってください。
...
はじめは `確認用 / 本番` を選ばせる作りにしていました。やめました。

選ばせると `Use workflow from`（どの枝の中身を出すか）と 2 つの選択になり、
**「作業中の枝の中身を本番に出す」が指定できてしまいます。**

出す先を枝から決めれば、その組み合わせは存在しません。**選べないものは間違えようがない**ので、
見張りを足すより選択肢を消すほうが確実です。
