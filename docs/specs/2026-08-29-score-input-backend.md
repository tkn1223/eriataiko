...
> **【2026-09-06 追記】この仕様書に書いてある役割分担はもう無効です。**
> 「フロントは別の人が担当する」「◯◯側の担当」と書いてある箇所がありますが、
> **役割分担はやめました。実装する人が最後まで通します**（`AGENTS.md` の「役割分担は無い」）。
> 分担して作った結果、「得点を入れる画面」と「得点を保存する入口」が両方できているのに
> **その 2 つをつなぐ人がいないまま**止まりました。
> 次に何をやるかは `docs/roadmap.md` を見てください。
