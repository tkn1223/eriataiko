...
| `matches` | `team_match_id` `division_id` `order_in_team_match` `court_number` `order_in_court` `status` `outcome` `started_at` `finished_at` |
...
- [ ] 1 つの対戦に何試合でも入る（試合数に上限が無い）
- [ ] 「何点先取」の列が無い（勝敗は得点の多いほうで決まる）
- [ ] 1 つの試合にゲームを何本でも入れられる
- [ ] 棄権・不戦勝を試合に記録できる
...
| 何ゲーム・何点 | **どこにも持たない** | 勝敗の判定に要らない（ゲームは点が多いほう、試合はゲームを多く取ったほう）。実際は「1部 21,21,11 / 2部 15,15,5」のようにゲームごとに違い、持たせると**試合の行ごとの設定作業**が増える。これまでも人の目で確かめてきた箇所 |
| 試合の終了 | **人が「確定」を押す** | 点数の上限が無いので自動で判定できない。最初の得点が入ったら「進行中」は自動 |
| 試合数 | **上限を設けない** | 1 人あたり 5 試合が目安（リーグ3・決勝2）だが、大会によって変わる。1 対戦あたり 3 試合のことも 8 試合のこともある |
